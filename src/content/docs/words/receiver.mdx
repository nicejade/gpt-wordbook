...
title: Receiver
description: >-
  Receiver 是一个名词，主要有三个含义：1. 接收器，如电话接收器；2. 接收人，指接受或收到某事物的人；3.
  （法律）接管人，指被指派来管理或处理破产或不法行为的公司的人。
sidebar:
  hidden: true
...
<Pronunciation word="Receiver" uk="/ɹəˈsiːvə/" us="/ɹəˈsivɚ/" client:only />
...
"Receiver"的動詞形式為 receive（主要且常用的形式）。

### 记忆辅助

你可以把"receiver"和"receive"一起记忆，想象一个情景，你是一个 terminal（终端），你接收（receive）信号，那么你就可以称为 receiver（接收器）。这样更方便记忆。
...
<WordRelations synonyms={["recipient"]} client:only />
