...
title: Path
description: >-
  Path 是一个英语名词，用来表示从一个地方到另一个地方的路或通道。例如， 'I walked along the forest path'
  (我沿着森林小径走过)。
sidebar:
  hidden: true
...
<Pronunciation word="Path" uk="[pʰɑːθ]" us="[pʰeəθ]" client:only />
...
Path 的词根来自古英语中的 "paþ"，这个词在古英语中同样表示路、路径。

### 词缀分析

Path 是一个词根,并无词缀，但可以添加词缀形成新词，比如"pathway"（foot path）,"footpath"（步行道）等。
...
<WordRelations synonyms={["track", "trail"]} client:only />
