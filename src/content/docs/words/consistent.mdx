...
title: Consistent
description: >-
  "Consistent" 是一个形容词，主要含有两种含义： 1. 维持一定水平、程度或速率的、一致的。例如："He has been a
  consistent supporter of the museum"（他一直是博物馆的坚定支持者）。 2.
  遵循相同的规则、行为或开展的，始终如一的。例如："His actions are consistent with his
  words"（他的行为与他的言辞一致）。
sidebar:
  hidden: true
