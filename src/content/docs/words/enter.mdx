...
description: >-
  Enter
  是一个常见的英文动词，它意味着'进入'或'开始'。这个词经常被用来形容物理上的进入（例如进入一个房间或建筑物）或抽象概念的开始（例如进入一个新的阶段或状态）。
sidebar:
  hidden: true
...
<Pronunciation word="Enter" uk="/ˈɛntə(ɹ)/" us="/ˈɛntɚ/" client:only />
...
自公元 14 世纪以来，"enter" 一词就已经被用来表示 "开始" 或 "进行" 的概念。此外，这个词也经常在电脑或其他设备的交互中使用，用来表示开始执行命令或操作。
...
<WordRelations synonyms={["ingo"]} antonyms={["exit"]} client:only />
