...
description: >-
  Timetable
  是一个名词。它的基本词义是一种时间表或者日程表。通常是列出列车、公交车或者飞机等公共交通工具的发车或起飞时间，或者列出学校或大学的课程表。具有计划和秩序的蕴含。
sidebar:
  hidden: true
...
Timetable 是一个复合词，由"Time"和 "Table"两个词组成。
...
"Timetable"这个词在 18 世纪中期虽然被提出，但直到 19 世纪初，随着火车和船只的出现，才开始广泛使用这个单词。
...
<WordRelations synonyms={["schedule", "timeline"]} client:only />
