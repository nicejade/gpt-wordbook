---
title: Order
description: Order 有两个主要的词义：1.命令、指示；2.整理、排序。它也可以作为一个动词使用，用于表示给出命令或整理的动作。
sidebar:
  hidden: true
...
Order 有两个主要的词义：1.命令、指示；2.整理、排序。它也可以作为一个动词使用，用于表示给出命令或整理的动作。

<Pronunciation word="Order" uk="/ˈɔːdə/" us="/ˈɔɹdɚ/" client:only />
...
Order 来自拉丁词根'ordo'，表示排列或顺序。有一些词，如'ordinary'（平凡的）、'ordinal'（序数的），都来自这个词根。

### 词缀分析

Order 是一个基本词，没有词缀。但可以形成词缀词，如'reorder'（重新排序）、'disorder'（混乱）等。

### 文化背景

Order 一词源于古罗马，当时罗马军队在编队行军时，必须按照一定的顺序或命令进行，因此'ordo'成了军队的关键词，后来逐渐发展为英语里的'order'。

### 单词变形

Order 的过去式和过去分词都是'ordered'，现在分词是'ordering'。词义变异主要基于使用的语境，例如,'order'用作动词时可以表示点餐，而用作名词时可以表示命令。
