---
title: Pass
description: 1.  作为动词，'pass' 的基本含义是 '通过'，比如 'pass a test'（通过一项测试）。
sidebar:
  hidden: true
...
<Pronunciation word="Pass" uk="[pʰäːs]" us="[pʰaːs]" client:only />
...
<WordRelations synonyms={["spend", "pledge", "promise", "vow"]} client:only />
