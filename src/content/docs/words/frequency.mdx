---
title: Frequency
description: Frequency 是一个英文名词，原义指的是“频率”，即某个事件或状态在一定时间内发生或表现的次数。也常用于描述一段时间内，某件事或某个观念的出现频度。
sidebar:
  hidden: true
...
- 形容词形式：frequent（频繁的），反义词为 infrequent（不经常的，稀少的）
- 副词形式：frequently（频繁地）
