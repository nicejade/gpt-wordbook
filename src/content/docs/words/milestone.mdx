...
description: >-
  单词 'milestone'
  是一个名词，用来描述在进展或发展过程中的重要阶段和事件。它的含义可以是实际的，如在建路的过程中，设置的那种用来标识距离的立碑；也可以是比喻的，如一段旅程、一项项目、一段生活等的关键或重要阶段。它也可用于描述人生中的重要事件，例如毕业、结婚、生子等。
sidebar:
  hidden: true
...
"milestone"是个合成词，由"mile"（英里）和"stone"（石头）两个词拼接而成。最早出现在公元 1709 年，用于表示“置于道路边用以标记每英里距离的石碑”。
...
<WordRelations synonyms={["landmark"]} client:only />
