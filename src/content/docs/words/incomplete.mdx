---
title: Incomplete
description: Incomplete是一个形容词，指的是“不完全的”，“不完整的”。它可以用来描述任何还未完成或不完备的情况。
sidebar:
  hidden: true
...
"Incomplete"这个词第一次在英语中出现是在 15 世纪，最初它被用来描述人的品质或性格方面的不完整或不完善。而在文化背景中，当一个事物被描述为“不完整”时，通常包含了对该事物还未实现其全部潜力或目标的含义，这也是一个对事物的继续改进和完成的期望。
...
<WordRelations
  synonyms={["underdone", "unfinished", "unwhole"]}
  antonyms={["complete"]}
  client:only
/>
