---
title: Unless
description: Unless 是一个连词，用于表示一个条件：除非……否则……。在许多情况下，它可以用于代替 'if not'。
sidebar:
  hidden: true
...
<Pronunciation word="Unless" uk="/ənˈlɛs/" us="/ənˈlɛs/" client:only />
...
"Unless" 这个词在英语中的运用有着悠久的历史。在 14 世纪时，“onlesse”就开始被短化为 "unless"。小小的一个词，承载着浓厚的历史文化信息，是英语中不可或缺的一部分。
...
<WordRelations antonyms={["if"]} client:only />
