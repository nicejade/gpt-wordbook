...
title: Check
description: >-
  "Check" 是一个非常常用的英文单词，既可以当动词，也可以当名词使用。 当 "check" 作为动词使用时，含义较多，比如： 1.
  检查、核实，例如：Check your work before handing it in (交上来之前检查一下你的工作)。 2. 控制、阻止，例如：The
  government attempted to check inflation (政府试图控制通货膨胀)。 3. 用棋将对方（在国际象棋里），例如：The
  King is in check (国王被将了)。 当 "check" 作为名词使用时，主要含义有： 1. 检查，例如：A routine health
  check (常规健康检查)。 2. 支票，例如：Can I pay by check? (我可以用支票付款吗？) 3. （在国际象棋中的）将，例如：I
  have a check. (我将了对方。)
sidebar:
  hidden: true
