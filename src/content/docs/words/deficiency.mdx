...
title: Deficiency
description: >-
  Deficiency 是一个名词，表示缺乏或不足，通常用于描述一种状态，其中某样东西的数量或质量低于正常或期望的水平。例如，它可以描述缺乏营养（例如，维生素
  D 缺乏）或其它的不足（例如，资金的缺乏）。
sidebar:
  hidden: true
...
"Deficiency" 是一个名词，表示缺乏或不足，通常用于描述一种状态，其中某样东西的数量或质量低于正常或期望的水平。例如，它可以描述缺乏营养（例如，维生素 D 缺乏）或其它的不足（例如，资金的缺乏）。
...
"Deficiency" 一词源自拉丁词根 "deficere"，首次在 15 世纪的英语中作为 "deficience" 出现，随后在 16 世纪演变为现在的 "Deficiency"。
...
<WordRelations antonyms={["excess", "sufficiency"]} client:only />
