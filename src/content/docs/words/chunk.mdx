...
description: >-
  Chunk
  是英语中的一个名词，通常用来描述一部分或者一个块，其准确的翻译取决于上下文。有时它可以描述一块物质（如一块石头），也可以表示一组信息或数据的部分（如一个文章的一小部分或者一块代码）。
sidebar:
  hidden: true
...
<Pronunciation word="Chunk" uk="/t͡ʃʌŋk/" us="/t͡ʃʌŋk/" client:only />
...
"Chunk"的起源不太清楚，但是它最早是在 17 世纪英国西南部地区的方言中出现的，最初用来形容一个切下来的一块或者一个大块。
