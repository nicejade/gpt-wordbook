...
title: Occurrence
description: >-
  Occurrence 是一个英文名词，主要用于指代'发生的事'或'事件”，在一些情况下也可表示'存在'或'出现'。如：'the occurrence of
  an accident' (意外的发生) 或 'an occurrence of gold' (黄金的存在)。
sidebar:
  hidden: true
...
<WordRelations antonyms={["state"]} client:only />
