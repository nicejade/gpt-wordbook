...
title: Composite
description: >-
  Composite 是一个英语单词，可以作为名词和形容词使用。作为形容词时，它的意思是由两种或更多的部分、元素或物质组合形成的。例如：'a
  composite picture' (一幅由多张图片组合而成的画面)。作为名词时，它指的是一个由多种部分或元素组合形成的事物，例如：'Aircraft
  are now made largely of composites' (现在的飞机大部分都是由复合材料制成的)。
sidebar:
  hidden: true
...
<Pronunciation word="Composite" uk="/ˈkɒmpəzɪt/" us="/kəmˈpɑzɪt/" client:only />
...
"Composite" 这个词最早来自于 14 世纪的拉丁语 "compositus"，在那个时候，它在建筑界被用来描述由古典（罗马和希腊）建筑风格元素复合而成的设计方案。
