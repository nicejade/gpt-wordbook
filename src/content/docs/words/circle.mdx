...
title: circle
description: >-
  Circle 是一个英语单词，主要有以下几个意思： 1. 圆圈，指一个点围绕中心移动形成的封闭线。例如：Draw a circle on the
  paper.（在纸上画一个圆圈） 2. 环，形容物体的外形像圆圈的对象。例如：The necklace is a gold
  circle.（项链是金制的环形） 3. 循环，代表一种思想、行为、事件的重复。例如：Life is a circle.（生活就是一个循环）
sidebar:
  hidden: true
