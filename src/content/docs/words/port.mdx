...
title: Port
description: >-
  Port作为名词有多种定义，包括：1.港口，一个设有设施以接收船只并处理其货物和乘客的地方。2.计算机接口，电子设备（如计算机）用于与其他设备连接或交互的接口。
sidebar:
  hidden: true
...
<Pronunciation word="Port" uk="/pɔːt/" us="/pɔɹt/" client:only />

### 列举例句

1.  The ship docked at the port. (船停靠在港口。)
2.  The USB port is not working. (USB 接口不工作。)
...
"Port"本身就是词根，没有词缀。它在很多单词中用作词根，如 importer（进口商），transportation（运输）等。
...
(约翰把他的设备插入 USB 端口。立刻，文件就开始像船只一样从他的设备移动到计算机，就像在繁忙的港口停靠。)

<WordRelations
  synonyms={["backboard","larboard","left","harbour","haven"]}
  antonyms={["starboard"]}
  client:only
/>
