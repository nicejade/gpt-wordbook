...
description: >-
  英文单词 'backslide'
  通常指由一种良好或振作的状态、习惯或者行为方式回滑到不佳或更差的状态。简单来说，就是退步或者逆行。比如：一个人决定戒烟，但过段时间却又开始吸烟，这就是
  backslide 的例子。
sidebar:
  hidden: true
...
Backslide 的词根是"back"和"slide"两个完整的英文单词。"Back"指的是向后，或者返回，"slide"则表示滑动、滑落。这两个单词组合在一起就形成了"backslide"，即"退后滑落"的意思。和"backslide"一样，都包含后退含义的单词还有例如"backtrack"、"backup"等。

### 词缀分析

Backslide 没有词缀，它由两个完整的单词，"back"和"slide"组成。
...
Backslide 是个动词, 其他时态: Backslid (过去时), Backslid (过去分词), Backsliding (现在分词)

### 记忆辅助

可以将 backslide 看作“back（后退）”和“slide（滑落）”的结合，类似于中文里的“后退”或“滑倒”，这样可以帮助记住它的含义及拼写。
