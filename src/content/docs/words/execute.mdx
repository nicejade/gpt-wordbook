...
title: Execute
description: >-
  Execute是一个动词，主要有两种含义。第一种是'执行'或'履行'，通常用于描述完成某项任务或遵守命令，例如'执行计划'，另一种意思则是'处决'，它是用来描述法律或政府机关对一项判决进行执行的行为。
sidebar:
  hidden: true
...
"Execute"来自中世纪拉丁文"executus"，过去分词形式的"exequi"，意为"执行，完成"，最早的使用可以追溯到 14 世纪。
...
<WordRelations synonyms={["launch", "open", "run", "start"]} client:only />
