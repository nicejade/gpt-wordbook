---
title: Outer
description: Outer 是一个形容词，表示 '在外部的' 或 '远离中心的'。它经常用来描述物资，地点，或者从一点到另一点的距离的边缘或者表面。
sidebar:
  hidden: true
...
<Pronunciation word="Outer" uk="/ˈaʊtə/" us="/ˈaʊtɚ/" client:only />
...
Outer 的词根是 "out-"，这个词根意为 "向外"。其他由此词根衍生的单词包括 "outwards"（向外的），"output"（产量）。

### 词缀分析

Outer 的词缀是后缀 "-er"，通常用于形容词或副词，表示更高级或者更极端。其他包含此词缀的单词包括 "faster"（更快的），"stronger"（更强的）。
...
<WordRelations antonyms={["inner"]} client:only />
