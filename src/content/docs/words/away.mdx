---
title: Away
description: Away 是一个英文副词，主要有以下几个意思：1)在远处或别处；2)离开，向外；3)不在，出门；4)消失，不见；5)连续，不断。
sidebar:
  hidden: true
...
<Pronunciation word="Away" uk="/əˈweɪ/" us="/əˈweɪ/" client:only />
...
<WordRelations synonyms={["off"]} client:only />
